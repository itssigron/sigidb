...
module.exports = (db, key, value, options) => {
    ...
    if (raw && key.includes(".")) {
        if(util.typeof(JSON.parse(raw[options.value])) !== "Object") throw new TypeError("Value of key in 'add' method with a target must be an object.");
        const oldVal = require("./get")(db,key,options);
        if(typeof oldVal !== "number") throw new Error("Target must be a number for db.add")

        obj = util.set({[key.split(".")[0]]: JSON.parse(raw[options.value])}, key, value + oldVal)[key.split(".")[0]];
    ...
    if (!raw) db.prepare(`INSERT INTO ${options.table} (${options.id}, ${options.value}) VALUES(?, ?)`).run(key.includes(".") ? key.split(".")[0] : key, value);
        ...
        console.log(key.includes(".") ? JSON.stringify(obj) : value, key.includes(".") ? key.split(".")[0] : key)

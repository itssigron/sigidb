...
module.exports = (db, key, value, options) => {
    ...
    if (raw && key.includes(".")) {
        ...
        value = util.set({ [key.split(".")[0]]: JSON.parse(raw.value) }, key, value)[key.split(".")[0]];
    ...
    if (!raw) db.prepare(`INSERT INTO ${options.table} (id, value) VALUES(?, ?)`).run(key.includes(".") ? key.split(".")[0] : key, JSON.stringify(value));
        ...
        db.prepare(`UPDATE ${options.table} SET value=? WHERE id=?`).run(JSON.stringify(value), key.includes(".") ? key.split(".")[0] : key);
    };

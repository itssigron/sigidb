...
module.exports = (db, key, value, options) => {
    ...
    if (raw && key.includes(".")) {
        ...
        obj = util.set({[key.split(".")[0]]: JSON.parse(raw.value)}, key, oldVal - value)[key.split(".")[0]];
    ...
    if (!raw) db.prepare(`INSERT INTO ${options.table} (id, value) VALUES(?, ?)`).run(key.includes(".") ? key.split(".")[0] : key, value);
    else {
